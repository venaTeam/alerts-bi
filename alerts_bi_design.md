...
The approved toolchain is recorded in the implementation blueprint, section 2. In short: Python 3.12 or later, `pyproject.toml` with a committed lockfile, an installable `alerts_bi` package, the official Elasticsearch Python client, a real SQL Server driver, the regular OpenAI Python SDK, `pytest`, `ruff` and `mypy`. SQLite and in-memory persistence substitutes remain forbidden, as does a second fixture system.

**The JavaScript implementation is preserved, not discarded.** It is tagged `javascript-mvp` and remains the behavioural reference until the Python implementation passes unit, integration and acceptance testing against the same fixtures. `test/fixtures/expected-results.json` is unchanged and stays the acceptance oracle: it was hand-authored from the fixture definitions and is not regenerated for the port, which is precisely what makes it able to detect a behavioural difference between the two implementations.

Language-neutral assets carry over untouched: the SQL migrations, the registry JSON Schema, the expected-results manifest, the Markdown documentation, and the Docker Compose stack. When the port is complete, nothing in the build, tests, mock seeding or runtime may require Node.js.
